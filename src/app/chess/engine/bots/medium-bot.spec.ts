...
describe('MediumBot', () => {
    ...
    fc.assert(
      fc.property(
        ...
        (seed, rounds) => {
          ...
          const bot = new MediumBot(mulberry32(seed ^ 0x1234abcd));
          for (const color of ['white', 'black'] as const) {
    ...
    fc.assert(
      fc.property(
        ...
        (seed, plies) => {
          ...
          if (engine.status.outcome !== 'ongoing') return;
          const bot = new MediumBot(mulberry32(seed ^ 0x9abcdef1));
          const color = engine.activeColor;
          const intent = bot.chooseMove(engine.position, color, engine);
    ...
    fc.assert(
      fc.property(
        ...
        (seed, rounds) => {
          const engine = randomSimultaneousPlayout(seed, rounds);
          if (engine.status.outcome !== 'ongoing') return;
          const bot = new MediumBot(mulberry32(seed ^ 0x51ab7e2d));
  ...
  it('refuses a "capture" that is defended by more attackers than it is worth', () => {
    // The black pawn on d4 is guarded by two more pawns (c5, e5); trading a
    // knight for a pawn there is a losing swap Medium should avoid when a
    // materially sound alternative exists.
